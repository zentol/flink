...
public class PrometheusReporter extends AbstractPrometheusReporter {
	...
	public PrometheusReporter(String portsConfig, boolean filterLabelValueCharacters) {
		super(filterLabelValueCharacters);

		Iterator<Integer> ports = NetUtils.getPortRangeFromString(portsConfig);
		HTTPServer httpServerCandidate  = null;
		int portCandidate = -1;
		while (ports.hasNext()) {
			int port = ports.next();
			try {
				// internally accesses CollectorRegistry.defaultRegistry
				httpServerCandidate = new HTTPServer(port);
				portCandidate = port;
