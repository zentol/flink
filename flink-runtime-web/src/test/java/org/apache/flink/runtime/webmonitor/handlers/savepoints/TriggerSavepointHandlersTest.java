...
public class TriggerSavepointHandlersTest {

	private static final ExecutionContext EC = ExecutionContexts$.MODULE$.fromExecutor(Executors.directExecutor());

	@Test
	public void testGetPaths() {
		TriggerSavepointHandlers handler = new TriggerSavepointHandlers(mock(ExecutionGraphHolder.class), EC, null);
	...
	public void testAskTimeoutEqualsCheckpointTimeout() throws Exception {
		...
		TriggerSavepointHandlers handlers = new TriggerSavepointHandlers(holder, EC, null);
		...
		verify(jobManager).ask(any(TriggerSavepoint.class), eq(FiniteDuration.apply(timeout, "ms")));
	}
	...
	public void testSavepointDirectoryConfiguration() throws Exception {
		...
		TriggerSavepointHandlers handlers = new TriggerSavepointHandlers(holder, EC, "the-default-directory");
		RequestHandler handler = handlers.getTriggerHandler();
		...
		verify(jobManager).ask(eq(new TriggerSavepoint(jobId, Option.apply("custom-directory"))), eq(FiniteDuration.apply(timeout, "ms")));
		...
		verify(jobManager).ask(eq(new TriggerSavepoint(jobId,  Option.apply("the-default-directory"))), eq(FiniteDuration.apply(timeout, "ms")));

		// 3. Throw Exception
		handlers = new TriggerSavepointHandlers(holder, EC, null);
	...
	public void testTriggerNewRequest() throws Exception {
		...
		verify(jobManager).ask(eq(new TriggerSavepoint(jobId, Option.apply("custom-directory"))), any(FiniteDuration.class));
