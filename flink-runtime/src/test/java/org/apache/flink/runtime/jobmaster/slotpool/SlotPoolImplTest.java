...
public class SlotPoolImplTest extends TestLogger {
    ...
    public void testAllocateSimpleSlot() throws Exception {
        ...
        try (SlotPoolImpl slotPool = createAndSetUpSlotPool(resourceManagerGateway)) {
            ...
            assertTrue(
                    registerAndOfferSlot(
                            taskManagerLocation, slotPool, slotRequest.getAllocationId()));
    ...
    public void testAllocationFulfilledByReturnedSlot() throws Exception {
        ...
        try (SlotPoolImpl slotPool = createAndSetUpSlotPool(resourceManagerGateway)) {
            ...
            final CompletableFuture<PhysicalSlot> future1 =
                    requestNewAllocatedSlot(slotPool, requestId1);
            final SlotRequestId requestId2 = new SlotRequestId();
            final CompletableFuture<PhysicalSlot> future2 =
                    requestNewAllocatedSlot(slotPool, requestId2);
            ...
            for (int i = 0; i < 2; i++) {
                slotRequests.add(
                        slotRequestQueue.poll(TIMEOUT.toMilliseconds(), TimeUnit.MILLISECONDS));
            }

            assertTrue(
                    registerAndOfferSlot(
                            taskManagerLocation, slotPool, slotRequests.get(0).getAllocationId()));
    ...
    public void testAllocateWithFreeSlot() throws Exception {
        ...
        try (SlotPoolImpl slotPool = createAndSetUpSlotPool(resourceManagerGateway)) {
            ...
            Optional<PhysicalSlot> physicalSlot =
                    slotPool.allocateAvailableSlot(
                            new SlotRequestId(), allocationId, ResourceProfile.ANY);
    ...
    public void testOfferSlot() throws Exception {
        ...
        try (SlotPoolImpl slotPool = createAndSetUpSlotPool(resourceManagerGateway)) {
            ...
            assertFalse(
                    slotPool.offerSlot(invalidTaskManagerLocation, taskManagerGateway, slotOffer));

            final SlotOffer nonRequestedSlotOffer =
                    new SlotOffer(new AllocationID(), 0, ResourceProfile.ANY);

            // we'll also accept non requested slots
            assertTrue(
                    slotPool.offerSlot(
                            taskManagerLocation, taskManagerGateway, nonRequestedSlotOffer));
            assertEquals(1, slotPool.getAllocatedSlots().size());
            ...
            final SlotOffer anotherSlotOfferWithSameAllocationId =
                    new SlotOffer(slotRequest.getAllocationId(), 1, ResourceProfile.ANY);
            assertFalse(
                    slotPool.offerSlot(
                            taskManagerLocation,
                            taskManagerGateway,
                            anotherSlotOfferWithSameAllocationId));

            TaskManagerLocation anotherTaskManagerLocation = new LocalTaskManagerLocation();
            assertFalse(
                    slotPool.offerSlot(anotherTaskManagerLocation, taskManagerGateway, slotOffer));
        ...
        try (SlotPoolImpl slotPool = createAndSetUpSlotPool(resourceManagerGateway)) {
            ...
            final SingleLogicalSlot logicalSlot =
                    SingleLogicalSlot.allocateFromPhysicalSlot(
                            requestId1, slot1, Locality.UNKNOWN, new DummySlotOwner(), true);
    ...
    /**
     * Tests that unused offered slots are directly used to fulfill pending slot requests.
     *
    ...
    public void testFulfillingSlotRequestsWithUnusedOfferedSlots() throws Exception {
        ...
        try (SlotPoolImpl slotPool = createAndSetUpSlotPool(resourceManagerGateway)) {
            ...
            final CompletableFuture<PhysicalSlot> slotFuture1 =
                    requestNewAllocatedSlot(slotPool, slotRequestId1);

            // wait for the first slot request
            final AllocationID allocationId1 = allocationIds.take();

            final CompletableFuture<PhysicalSlot> slotFuture2 =
                    requestNewAllocatedSlot(slotPool, slotRequestId2);
    ...
    public void testShutdownReleasesAllSlots() throws Exception {
        ...
        try (SlotPoolImpl slotPool = createAndSetUpSlotPool(resourceManagerGateway)) {
            ...
            final ArrayBlockingQueue<AllocationID> freedSlotQueue =
                    new ArrayBlockingQueue<>(numSlotOffers);
            ...
            assertThat(
                    ...
                    Matchers.containsInAnyOrder(
                            slotOffers.stream().map(SlotOffer::getAllocationId).toArray()));
        }
    }
    ...
    public void testCheckIdleSlot() throws Exception {
        ...
        try (TestingSlotPoolImpl slotPool =
                createAndSetUpSlotPool(resourceManagerGateway, clock, TIMEOUT)) {
            ...
            assertThat(
                    slotPool.offerSlot(taskManagerLocation, taskManagerGateway, slotToNotExpire),
                    Matchers.is(true));
    ...
    public void testDiscardIdleSlotIfReleasingFailed() throws Exception {
        ...
        try (TestingSlotPoolImpl slotPool =
            ...
            final SlotOffer slotToExpire =
                    new SlotOffer(expiredAllocationId, 0, ResourceProfile.ANY);
            ...
            taskManagerGateway.setFreeSlotFunction(
                    (AllocationID allocationId, Throwable cause) -> {
                        ...
                        return FutureUtils.completedExceptionally(
                                new TimeoutException("Test failure"));
                    });
            ...
            final CompletableFuture<PhysicalSlot> allocatedSlotFuture =
                    requestNewAllocatedSlot(slotPool, new SlotRequestId());
    ...
    public void testFreeFailedSlots() throws Exception {
        ...
        try (SlotPoolImpl slotPool = createAndSetUpSlotPool(resourceManagerGateway)) {
            ...
            final Map<SlotRequestId, CompletableFuture<PhysicalSlot>> slotRequestFutures =
                    new HashMap<>(parallelism);

            for (int i = 0; i < parallelism; i++) {
                final SlotRequestId slotRequestId = new SlotRequestId();
                slotRequestFutures.put(
                        slotRequestId, requestNewAllocatedSlot(slotPool, slotRequestId));
            }
    ...
    public void testCreateAllocatedSlotReport() throws Exception {
        ...
        try (SlotPoolImpl slotPool = createAndSetUpSlotPool(resourceManagerGateway, jobId)) {
            ...
            assertThat(
                    slotReport.getAllocatedSlotInfos(),
    ...
    public void testCalculationOfTaskExecutorUtilization() throws Exception {
        try (final SlotPoolImpl slotPool = createAndSetUpSlotPool(resourceManagerGateway)) {
            ...
            slotPool.allocateAvailableSlot(
                    new SlotRequestId(), firstTaskManagersSlots.get(0), ResourceProfile.ANY);
            slotPool.allocateAvailableSlot(
                    new SlotRequestId(), firstTaskManagersSlots.get(1), ResourceProfile.ANY);
            slotPool.allocateAvailableSlot(
                    new SlotRequestId(), secondTaskManagersSlots.get(3), ResourceProfile.ANY);

            final Collection<SlotInfoWithUtilization> availableSlotsInformation =
            ...
            for (SlotInfoWithUtilization slotInfoWithUtilization : availableSlotsInformation) {
                final double expectedTaskExecutorUtilization =
                        utilizationPerTaskExecutor.get(
                                slotInfoWithUtilization.getTaskManagerLocation());
                assertThat(
                        slotInfoWithUtilization.getTaskExecutorUtilization(),
                        is(closeTo(expectedTaskExecutorUtilization, 0.1)));
            }
        }
    }
    ...
    public void testOrphanedAllocationIsCanceledIfNotRemapped() throws Exception {
        ...
        try (SlotPoolImpl slotPool = SlotPoolUtils.createAndSetUpSlotPool(resourceManagerGateway)) {
            ...
            } while (randomAllocationId.equals(allocationId1)
                    || randomAllocationId.equals(allocationId2));
    ...
    private static TestingSlotPoolImpl createAndSetUpSlotPool(
            final ResourceManagerGateway resourceManagerGateway, final JobID jobId)
            throws Exception {
    ...
    private List<AllocationID> registerAndOfferSlots(
        ...
        final List<AllocationID> allocationIds =
                IntStream.range(0, numberOfSlotsToRegister)
    ...
    private static Collection<Matcher<? super AllocatedSlotInfo>> isEachEqual(
            Collection<AllocatedSlotInfo> allocatedSlotInfos) {
        return allocatedSlotInfos.stream()
                .map(SlotPoolImplTest::isEqualAllocatedSlotInfo)
                .collect(Collectors.toList());
    }

    private static Matcher<AllocatedSlotInfo> isEqualAllocatedSlotInfo(
            AllocatedSlotInfo expectedAllocatedSlotInfo) {
        return new TypeSafeDiagnosingMatcher<AllocatedSlotInfo>() {
            ...
            private String describeAllocatedSlotInformation(
                    AllocatedSlotInfo expectedAllocatedSlotInformation) {
                return expectedAllocatedSlotInformation.toString();
            }

            @Override
            protected boolean matchesSafely(
                    AllocatedSlotInfo item, Description mismatchDescription) {
                final boolean matches =
                        item.getAllocationId().equals(expectedAllocatedSlotInfo.getAllocationId())
                ...
                if (!matches) {
                    mismatchDescription
                            ...
                            .appendText(
                                    describeAllocatedSlotInformation(expectedAllocatedSlotInfo));
                }
