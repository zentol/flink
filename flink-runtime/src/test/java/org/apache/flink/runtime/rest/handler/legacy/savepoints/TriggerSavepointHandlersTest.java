...
/**
 * Tests for the {@link TriggerSavepointHandlers}.
 */
public class TriggerSavepointHandlersTest extends TestLogger {

	private static final Executor executor = Executors.directExecutor();

	@Test
	public void testGetPaths() {
		TriggerSavepointHandlers handler = new TriggerSavepointHandlers(mock(ExecutionGraphCache.class), executor, null);
		...
		Assert.assertTrue(triggerPathsList.contains("/jobs/:jobid/trigger-savepoint"));
		Assert.assertTrue(triggerPathsList.contains("/jobs/:jobid/trigger-savepoint/target-directory/:targetDirectory"));
		...
		Assert.assertEquals("/jobs/:jobid/trigger-savepoint/in-progress/:requestId", progressPaths[0]);
	}
	...
	public void testAskTimeoutEqualsCheckpointTimeout() throws Exception {
		...
		TriggerSavepointHandlers handlers = new TriggerSavepointHandlers(holder, executor, null);
		...
		when(jobManager.triggerSavepoint(eq(jobId), anyString(), any(Time.class))).thenReturn(CompletableFuture.completedFuture("foobar"));

		handler.handleRequest(params, Collections.emptyMap(), jobManager);

		verify(jobManager).triggerSavepoint(eq(jobId), anyString(), any(Time.class));
	}
	...
	public void testSavepointDirectoryConfiguration() throws Exception {
		...
		TriggerSavepointHandlers handlers = new TriggerSavepointHandlers(holder, executor, "the-default-directory");
		RequestHandler handler = handlers.getTriggerHandler();
		...
		verify(jobManager).triggerSavepoint(eq(jobId), eq("custom-directory"), any(Time.class));
		...
		verify(jobManager).triggerSavepoint(eq(jobId), eq("the-default-directory"), any(Time.class));

		// 3. Throw Exception
		handlers = new TriggerSavepointHandlers(holder, executor, null);
	...
	public void testTriggerNewRequest() throws Exception {
		...
		when(jobManager.triggerSavepoint(eq(jobId), eq("custom-directory"), any(Time.class))).thenReturn(successfulCancelWithSavepoint);
		...
		String location = String.format("/jobs/%s/trigger-savepoint/in-progress/1", jobId);
	...
	public void testFailedCancellation() throws Exception {
		...
		when(jobManager.triggerSavepoint(eq(jobId), eq("custom-directory"), any(Time.class))).thenReturn(unsuccessfulCancelWithSavepoint);
